...
from PitchEstimationDataSet_cp import *
# Training settings
...
def test():
    ...
    for data, target in test_loader:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data, volatile=True).type(torch.ByteTensor), Variable(target).type(torch.FloatTensor)
        output = model(data)
